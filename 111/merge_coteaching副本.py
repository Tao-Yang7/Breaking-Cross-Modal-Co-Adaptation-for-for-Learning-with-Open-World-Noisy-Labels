...
def main():
    ...
    positive_prompt_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(positive_prompt_optimizer, T_max=80, eta_min=1e-4)

    negative_prompt_optimizer = torch.optim.SGD(negative_prompt_params, lr=5e-4, momentum=0.9)  # Match original settings
    negative_prompt_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(negative_prompt_optimizer, T_max=80, eta_min=1e-4)
    ...
    ##### initialize wandb
    import wandb
    run = wandb.init(project="cifar80N", dir='.', reinit=True, name="sym0.2",
        config={
    ...
    for epoch in range(args.max_epoch):
        ...
        if epoch_cycle < 2:
            ...
            if results['ID']['acc'] > best_acc_visual:
                ...
                path = f"/data/tyang/checkpoints/cifar80N/sym0.2/best_acc_{timestamp}_1225.pth"
